        ...
        <div class="col-md-6">
          ...
          <div class="box box-primary">
            ...
            @endif
             ...
             <form method="post" action="{{ url('admin/manage-zone-questions/add') }}" enctype="multipart/form-data">
              ...
              <div class="box-body">
                <div class="form-group">
                  <label for="choose_zone_id">Select Zone</label>
                  
                  <select class="form-control" name="choose_zone_id" disabled>
                    <option>Select Zone</option>
                    @if($zone_list)
                    @foreach($zone_list as $zone)
                    <option value="{{ $zone->id }}" {{ (old("choose_zone_id",@$zone_id) == $zone->id ? "selected":"") }}>{{ $zone->zone_name}}</option>
                    @endforeach
                    @endif
                  </select>
                  <span class="text-danger">{{ $errors->first('choose_zone_id') }}</span>
                </div>
                <div class="form-group">
